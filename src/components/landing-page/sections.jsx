    ...
    <>
      <section id="features" className="max-w-7xl mx-auto px-6 py-20 md:py-28">
        <m.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.3 }} variants={v.container}>
          ...
          <m.h2 variants={v.item} className="font-display font-bold uppercase text-3xl md:text-4xl text-foreground mb-10">
            Fuel that follows your training
          </m.h2>
        </m.div>
        <m.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.3 }} variants={cardStagger}
          className="grid sm:grid-cols-3 gap-5">
          {FEATURES.map(({ icon: Icon, tile, iconClass, stat, title, description }) => (
            <m.div key={title} variants={v.item} className="bg-card border border-border rounded-2xl p-5">
      ...
      <section className="bg-card border-y border-border">
        <m.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.4 }} variants={v.container}
          ...
          <m.h2 variants={v.item} className="font-display font-bold uppercase text-3xl md:text-4xl text-foreground">
